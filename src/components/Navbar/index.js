      ...
      {path.includes("admin") ? (
        ...
        <div>
          <nav className="w-full">
            ...
            <div className="bg-black text-white">
             ...
             <a>{svgIcon}</a> */}
                ...
                <div className="flex">
                  <Link
                    className="text-xs px-4 py-2 leading-none rounded-full hover:bg-teal-300 hover:text-black border-2"
                    href="\loginPage"
            ...
            <div className="w-full h-[20%] bg-white flex justify-center">
              <div className="container flex flex-col md:flex-row justify-between w-full md:w-[90%] p-5 gap-10 items-center">
                ...
                <div className="md:w-1/2 flex flex-col justify-center text-center">
                  <h5 className="text-teal-500 text-2xl md:text-3xl font-extrabold">
                    PRATIBHA GLOBAL SCHOOL GWALIOR
                  </h5>
                  <h5 className="text-sm md:text-base">
                    MP BOARD: DICE CODE 23040409413
                  </h5>
                </div>
          ...
          <div className="lg:flex justify-center max-w-full content-center hidden md:flex  w-full ">
            <div
              className={`bg-teal-500 h-12 rounded-b-lg flex justify-around w-[100%]  text-center ${
                isMenuOpen ? "flex-col" : ""
              }`}
            >
              <Link
                className="text-base font-extrabold text-[13px] px-6 py-4 leading-none hover:bg-white"
                href="/"
              >
                HOME
              </Link>
              ...
              <Link
                className='text-base font-extrabold text-[13px] px-6 py-4 leading-none hover:bg-white flex" href="/contact-us'
                href="/contact-us"
              >
                CONTACT
              </Link>
                          ...
                          <ul className="">
                            {values.map((value, index) => (
                              <li key={index} className="py-2">
                                ...
                                <div
                                  onClick={() =>
                                    handleSidebarItemClick(value.path)
                                  }
                                  className="p-3 shadow-md rounded-lg bg-teal-100 text-teal-600"
                                >
                                  {value.displayName}
                                </div>
                              </li>
...
function DropdownNavItem({ name, values }) {
  return (
    <div className="relative group">
      ...
      <div className="hidden text-start group-hover:block absolute z-10 bg-gradient-to-r from-teal-400 to-teal-100 p-5 rounded-lg w-64">
        <ul className="flex flex-col  ">
          {values.map((value, index) => (
            <Link href={value.path}>
              <li
                key={index}
                className="p-5 hover:bg-white rounded-lg font-extrabold text-[13px]"
              >
...
let imageData = [
  "https://lnctworldschools.com/wp-content/uploads/2023/07/lnctlogobs.png",
];
...
let dropdownItems = [
  {
    ...
    values: [
      ...
      {
        displayName: "Message From the Founder",
        path: "/about/founders-message",
      },
      {
        displayName: "Message From the Chairperson",
        path: "/about/chairmans-message",
      },
      {
        displayName: "Message From the Managing Director",
